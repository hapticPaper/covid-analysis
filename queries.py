...
UPDATE_KEY = """UPDATE daily SET combinedkey = replace(replace(case 
			when admin2 is not null then concat(admin2, ', ', provincestate,', ', countryregion)
			when provincestate is not null then concat(provincestate,', ', countryregion) 
	  		else countryregion
...
LAST_UPDATE = """SELECT max(lastupdate) lu FROM daily"""

DATES=f"""SELECT distinct date(lastupdate) from  daily where combinedkey like '%%US' order by 1"""
...
def DAILY_UPDATE(datey): 
    ...
    from fa_daily  where combinedkey like '%%US'  and date(lastupdate)='{datey}' group by  1, 2, combinedkey
    """
